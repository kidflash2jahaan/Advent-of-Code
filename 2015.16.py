all_aunts_string = """Sue 1: goldfish: 9, cars: 0, samoyeds: 9
...
correct_aunt_string = """children: 3
cats: 7
samoyeds: 2
pomeranians: 3
akitas: 0
...
correct_aunt = {}
for line in correct_aunt_string.replace(":", "").split("\n"):
    correct_aunt[line.split(" ")[0]] = int(line.split(" ")[1])

all_aunts = []
for line in all_aunts_string.replace(":", "").replace(",", "").split("\n"):
    all_aunts.append({line.split(" ")[2]: int(line.split(" ")[3]), line.split(" ")[4]: int(line.split(" ")[5]), line.split(" ")[6]: int(line.split(" ")[7])})

i = 0
for aunt in all_aunts:
    i += 1
    for key, value in correct_aunt.items():
        if key not in aunt:
            aunt[key] = value
    if aunt == correct_aunt:
        break
